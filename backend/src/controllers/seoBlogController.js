...
exports.suggestIdeas=async(req,res)=>{try{ok(res,await service.suggestIdeas(req.body,req.user.id));}catch(e){fail(res,e)}};
exports.listIdeas=async(req,res)=>{try{ok(res,await service.listIdeas(req.query.status||'unused'));}catch(e){fail(res,e)}};
exports.updateIdea=async(req,res)=>{try{const idea=await service.updateIdea(req.params.id,req.body);if(!idea)return res.status(404).json({success:false,message:'Không tìm thấy ý tưởng'});ok(res,idea);}catch(e){fail(res,e)}};
exports.deleteIdea=async(req,res)=>{try{if(!await service.deleteIdea(req.params.id))return res.status(404).json({success:false,message:'Không tìm thấy ý tưởng'});ok(res,{id:req.params.id});}catch(e){fail(res,e)}};
...
exports.imageSearch=async(req,res)=>{try{ok(res,await service.findImages(req.query.q,req.query.topic,Math.min(12,Math.max(1,+req.query.limit||8)),Math.min(20,Math.max(1,+req.query.page||1))));}catch(e){fail(res,e)}};
