...
} = require("./examReviewPrompt");
const { buildStoredExamReviewContext } = require("./examReviewRunner");
const { getSourceAnswerFixSkipReason } = require("./examFixApplier");
...
function buildReviewPrompt(batch, context = {}) {
  ...
  const sourceRules = buildSourceReviewRules(context.sourceFile);
...
function normalizeAiFix(rawFix, entry) {
  ...
  const normalized = {
    ...
    suggestedCorrectAnswer: stringValue(entry.review?.suggestedCorrectAnswer).toUpperCase().slice(0, 1),
...
async function applyExamReviewFixes(client, examId, reviews = [], options = {}) {
  ...
  const reviewByPath = new Map(
    (Array.isArray(reviews) ? reviews : [])
      .filter(review => review?.path)
      .map(review => [review.path, review]),
  );
  const generated = await generateReviewFixesWithAI(entries, reviews, {
    ...reviewContext,
  });
  if (sourceFile && generated.summary) {
    generated.summary.sourceFileId = sourceFile.id;
    generated.summary.sourceFileName = sourceFile.fileName;
  }
