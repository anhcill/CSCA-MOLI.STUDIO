...
router.get('/',c.adminList); router.post('/',c.create); router.post('/generate',c.generate); router.post('/suggest-ideas',c.suggestIdeas); router.get('/ideas',c.listIdeas); router.patch('/ideas/:id',c.updateIdea); router.delete('/ideas/:id',c.deleteIdea); router.get('/image-search',c.imageSearch); router.post('/validate',c.validate);
