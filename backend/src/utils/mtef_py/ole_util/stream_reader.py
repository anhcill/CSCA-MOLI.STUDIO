...
class StreamReader:
    ...
    def read(self, read_size=None):
        ...
        while reaminLen > self.size_sector-self.offset_in_sector:
            ...
            if read_bytes is None or len(read_bytes) != to_read_size:
                ...
                if self.offset_of_sector >= len(self.sat):
                    pass # print('THIS SHOULD NOT HAPPEN, IF YOUR PROGRAM BREAK, COMMENT THIS LINE TO CONTINUE AND MAIL ME XLS FILE TO TEST, THANKS')
                    return b''.join(ans)
                else:
        ...
        if read_bytes and len(read_bytes) == read_size-readed:
            ...
            if DEBUG:
                pass # print('pos:%d,bit:' % self.offset_of_sector)
                pass # print(b''.join(ans))
            return b''.join(ans) 
        else:
            return b''.join(ans)
