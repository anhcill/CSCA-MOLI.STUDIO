  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {form.accessType === 'package' ? (
        <fieldset className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5">
          <legend className="px-2 font-black text-indigo-950">Gói thanh toán mở khóa học</legend>
          <p className="mb-4 text-sm text-slate-600">Học viên mua bất kỳ gói nào được chọn bên dưới sẽ được truy cập khóa học.</p>
          {packagesLoading ? <p className="text-sm text-slate-500">Đang tải danh sách gói...</p> : packagesError ? <p role="alert" className="text-sm font-semibold text-red-600">{packagesError}</p> : packages.length === 0 ? <p className="text-sm text-amber-700">Chưa có gói thanh toán đang hoạt động.</p> : (
            <div className="grid gap-3 sm:grid-cols-2">
              {packages.map((pkg) => (
                <label key={pkg.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border bg-white p-4 ${form.packageIds.includes(pkg.id) ? 'border-indigo-500 ring-2 ring-indigo-100' : 'border-slate-200'}`}>
                  <input type="checkbox" checked={form.packageIds.includes(pkg.id)} onChange={() => togglePackage(pkg.id)} className="mt-1 h-5 w-5 rounded" />
                  <span>
                    <span className="block font-bold text-slate-800">{pkg.name}</span>
                    <span className="text-xs text-slate-500">{Number(pkg.price || 0).toLocaleString('vi-VN')}đ · {pkg.duration_days} ngày</span>
                  </span>
                </label>
              ))}
            </div>
