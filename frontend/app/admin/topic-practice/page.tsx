    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-5xl overflow-y-auto bg-[#f8fafc] p-4 shadow-2xl dark:bg-[#07111f] sm:p-6">
        ...
        <div className="mb-5 grid gap-3 rounded-2xl border border-red-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-[#111b2d] md:grid-cols-[1fr_auto] md:items-end">
          ...
          <p className="md:col-span-2 text-xs font-semibold leading-5 text-slate-500 dark:text-slate-300">Trình tự: tải PDF → lưu đủ đáp án → gán một chủ đề → đăng. Sau khi có người làm, hệ thống khóa đổi PDF, đáp án và chủ đề để bảo toàn thống kê.</p>
        </div>
...
export default function AdminTopicPracticePage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setItems(await examAdminApi.getTopicPracticeOverview(subjectFilter || undefined));
    } catch {
      setItems([]);
    } finally {
      setLoading(false);
  ...
  const createPracticeFile = async () => {
    ...
    if (!newTitle.trim() || !subject) return alert('Nhập tên file và chọn môn học.');
    try {
      setCreating(true);
      ...
      setEditorFile({ examId: result.exam.id, title: result.exam.title, subjectId: subject.id, subjectCode: subject.code, subjectName: subject.name, status: result.exam.status });
      setShowCreate(false);
      setNewTitle('');
      setNewDescription('');
      setNewSubjectId('');
  ...
  return (
    <AdminLayout title="Luyện chủ đề" description="Đăng file PDF, đáp án và theo dõi người học theo từng chủ đề">
      <section className="space-y-5">
        <div className="rounded-3xl border border-red-100 bg-gradient-to-br from-white via-rose-50 to-red-50 p-6 shadow-sm dark:border-slate-800 dark:from-[#111b2d] dark:via-[#101c31] dark:to-[#1d1730] dark:shadow-none">
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="inline-flex items-center gap-2 rounded-full bg-red-100 px-3 py-1.5 text-xs font-black uppercase tracking-wide text-red-700 dark:bg-red-500/15 dark:text-red-200"><FiTarget /> Quản lý kho luyện</p><h2 className="mt-3 text-2xl font-black text-slate-950 dark:text-white">PDF → đáp án → chủ đề → thống kê</h2><p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-600 dark:text-slate-300">Mỗi file chỉ xuất hiện cho học viên khi đã có PDF, đáp án đúng đầy đủ, chủ đề cùng môn và trạng thái đã đăng.</p></div><div className="flex flex-wrap gap-3"><Link href="/admin/risk-center" className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-white/75 px-4 py-3 text-sm font-black text-red-700 hover:bg-red-50 dark:border-red-500/35 dark:bg-slate-950/40 dark:text-red-200 dark:hover:bg-red-500/10"><FiAlertCircle /> Risk Center</Link><button type="button" onClick={() => setShowCreate(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-red-600 px-5 py-3 text-sm font-black text-white hover:bg-red-700"><FiPlus /> Tạo file luyện</button></div></div>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{([
          ['File đã cấu hình', totals.files, FiFileText, 'text-red-600 dark:text-red-300'],
          ['Đang đăng', totals.published, FiCheck, 'text-emerald-600 dark:text-emerald-300'],
