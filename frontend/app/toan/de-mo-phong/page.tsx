...
export default function ToanDeMoPhongPage() {
    ...
    const colorScheme = { from: 'from-blue-600', via: 'via-indigo-600', to: 'to-purple-700' };
    const { user } = useAuthStore();
    const isAdmin = hasPermission(user, 'exams.manage');

    return (
        <div className="min-h-screen bg-slate-50 relative overflow-hidden">
            <div className={`absolute top-0 left-1/4 w-full h-[500px] bg-gradient-to-br from-blue-600 opacity-5 blur-[120px] pointer-events-none`} />
            
            <Header />

            <main className="container mx-auto px-4 md:px-6 py-6 md:py-8 max-w-[1400px] relative z-10">
                <div className={`w-full rounded-[2rem] bg-gradient-to-r ${colorScheme.from} ${colorScheme.via} ${colorScheme.to} p-8 lg:p-12 shadow-xl shadow-indigo-900/10 relative overflow-hidden text-white flex flex-col md:flex-row items-center justify-between gap-8 mb-8`}>
                        ...
                        <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-gray-100 shadow-sm px-6 py-5 flex items-center justify-between">
                            ...
                            {isAdmin && (
                                <Link
                                    ...
                                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-violet-500/20 hover:shadow-violet-500/40 hover:-translate-y-0.5 transition-all"
                                >
