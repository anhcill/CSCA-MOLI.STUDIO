...
export function LearningRoomClient({ courseSlug, lessonId }: { courseSlug: string; lessonId: number }) {
  ...
  if (error || !room) return <div className="flex min-h-screen items-center justify-center bg-slate-100 p-8 dark:bg-slate-950"><div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center text-red-700 dark:border-red-900/70 dark:bg-red-950/30 dark:text-red-300"><p>{error || 'Không tìm thấy bài học.'}</p><button type="button" onClick={() => setRequestVersion((value) => value + 1)} className="mt-4 rounded-lg bg-red-600 px-4 py-2 font-bold text-white hover:bg-red-700">Thử lại</button></div></div>;

  return <div className="flex min-h-screen flex-col bg-slate-100 text-slate-950 transition-colors dark:bg-slate-950 dark:text-slate-100 lg:h-screen lg:overflow-hidden"><LearningHeader courseSlug={courseSlug} courseTitle={room.course.title} progressPct={room.enrollment.progress.completionPct} /><div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_380px]"><main className="min-h-0 lg:overflow-y-auto">{room.lesson.lessonType === 'video' ? <HlsPlayerShell lessonId={lessonId} session={session} loading={playbackLoading} error={playbackError} /> : null}<article className="mx-auto max-w-5xl p-6"><h2 className="text-3xl font-black text-slate-950 dark:text-white">{room.lesson.title}</h2>{room.lesson.summary ? <p className="mt-2 text-slate-500 dark:text-slate-400">{room.lesson.summary}</p> : null}<div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200">{room.lesson.contentHtml ? <div className="whitespace-pre-wrap">{room.lesson.contentHtml}</div> : <p>Nội dung bài học đang được cập nhật.</p>}</div></article></main><div className="min-h-0 lg:block"><LearningSidebar courseSlug={courseSlug} sections={room.curriculum} activeLessonId={lessonId} /></div></div><LearningFooter courseSlug={courseSlug} previousLessonId={room.lesson.previousLessonId} nextLessonId={room.lesson.nextLessonId} /></div>;
}
