        ...
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            {POLICIES.slice(0, 2).map(p => (
              <Link key={p.name} href={p.href} className="text-xs text-gray-600 hover:text-gray-400 transition-colors">{p.name}</Link>
            ))}
          </div>
          <p className="text-xs text-gray-700">Made with ❤️ in Vietnam 🇻🇳</p>
        </div>
      </div>
    </footer>
  );
