...
const selectClass =
  'min-h-12 w-full rounded-xl border border-[#ded2c7] bg-[#fffdf9] px-4 text-sm font-bold text-[#382f2a] outline-none transition focus:border-[#a87955] focus:ring-4 focus:ring-[#b78a68]/10';
...
export function CourseFilters({ value, onChange }: { value: CourseCatalogQuery; onChange: (next: CourseCatalogQuery) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-[#ddcfc0] bg-[#fffaf5]/95 p-4 shadow-[0_16px_36px_-28px_rgba(67,46,30,.55)] backdrop-blur sm:p-5">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <label className="grid gap-1.5 text-xs font-bold text-[#786a60]">
          <span className="inline-flex items-center gap-2"><FiBookOpen className="text-[#a46b45]" /> Môn học</span>
        ...
        <label className="grid gap-1.5 text-xs font-bold text-[#786a60]">
          <span className="inline-flex items-center gap-2"><FiLayers className="text-[#a46b45]" /> Hình thức truy cập</span>
        ...
        <label className="grid gap-1.5 text-xs font-bold text-[#786a60]">
          <span className="inline-flex items-center gap-2"><FiSliders className="text-[#a46b45]" /> Sắp xếp</span>
